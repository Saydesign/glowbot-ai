...
export function ChatWindow({ userName, skinType, onReset, onGoToAdmin }: ChatWindowProps) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const { data } = await supabase.from('products').select('id, name, category, price');
  ...
  // Find mentioned products in bot response
  ...
  const handleSendMessage = async (content: string) => {
    ...
    // Find products mentioned in the response
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-rose-50 flex flex-col relative">
                ...
                <div className="mb-4 ml-4 md:ml-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {message.products.map((product) => (
                      <div
                        key={product.id}
                        className="bg-white rounded-xl p-3 border border-pink-100 shadow-sm flex items-center gap-3"
                      >
                        <div>
                        ...
                        <button
                          onClick={() => addItem({ id: product.id, name: product.name, category: product.category, price: product.price })}
                          className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-pink-500 to-rose-400 text-white text-xs font-medium hover:shadow-md transition-all whitespace-nowrap"
                        >
                          + Keranjang
                        </button>
                      </div>
      ...
      {totalItems > 0 && (
        <button
          ...
          className="fixed bottom-20 right-4 w-14 h-14 rounded-full bg-gradient-to-r from-pink-500 to-rose-400 text-white shadow-lg flex items-center justify-center hover:shadow-xl transition-all z-40"
        >
          <ShoppingBag className="w-6 h-6" />
          <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-white text-pink-500 text-xs font-bold flex items-center justify-center">
            {totalItems}
          </span>
        </button>
      )}
